...
#        self.general_vb.setVerticalSpacing(0)
        ...
        self.build_apgrid.addWidget(QtGui.QLabel('AP Database is read only:'), 0, 0, 1, columnSpan = 2) # cols 1-2
        self.build_apgrid.addWidget(self.build_apronly, 0, 1)
        ...
        self.build_apgrid.addWidget(self.build_cmd, 1, 1, 1, columnSpan = 2) # cols 1-2
        self.build_gdlinc = FileEntry(self.build_apctrls, configval(config, 'build', 'makegdlfile'), 'GDL Files (*.gdl *.gdh)')
        self.build_apgrid.addWidget(QtGui.QLabel('Autogenerated GDL file:'), 2, 0)
        self.build_apgrid.addWidget(self.build_gdlinc, 2, 1, 1, columnSpan = 2) # cols 1-2
        self.build_pos = PassSpin(self.build_apctrls)
        pospass = configval(config, 'build', 'pospass')
        self.build_pos.setValue(int(pospass))
        self.build_apgrid.addWidget(QtGui.QLabel('Attachment positioning pass:'), 3, 0, 1, columnSpan = 2) # cols 1-2
        self.build_apgrid.addWidget(self.build_pos, 3, 1)
        ...
        # sub-controls for tweaking, enabled or disabled as a unit
        self.build_twkctrls = QtGui.QWidget(self.build) 
        self.build_vb.addWidget(self.build_twkctrls, 5, 0, 1, 3)
        self.build_twkgrid = QtGui.QGridLayout(self.build_twkctrls)
        self.build_twkgdl = FileEntry(self.build_twkctrls, configval(config, 'build', 'tweakgdlfile'), 'GDL Files (*.gdl *.gdh)')
        self.build_twkgrid.addWidget(QtGui.QLabel('Autogenerated GDL file:'), 5, 0)
        ...
        self.build_vb.setRowStretch(7, 1)
    ...
    def apChanged(self, txt) :
        ...
        if txt and (not self.build_pos.text() or self.build_pos.text() == 'None') :
            # Pass number must be at least 1.
            self.build_pos.setValue(1)

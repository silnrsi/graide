    ...
    def attribModel(self) :
        ...
        for pair in (('index', 'index'), ('glyph number','gid'), ('slot ID', 'id'), 
            ...
            if hasattr(self, attr) :
                res.append(Attribute(label, self.__getattribute__, None, False, None, None, False, attr))
        for k in ('origin', 'advance', 'shift') :
            if hasattr(self, k) :
                res.append(Attribute(k, self.getPos, None, False, None, None, False, k))
        for k in ('before', 'after') :
            res.append(Attribute(k, self.getCharInfo, None, False, None, None, False, k))
        ...
        if hasattr(self, 'parent') :
            res.append(Attribute('parent slot', self.getParent, None, False, None, None, False, 'parent'))
        ...
        if hasattr(self, 'collision') :
            ...
            resAttrib.add(Attribute('collision', None, None, True, None, None, False, cAttrib))
            if len(crem) :
                cremAttrib = AttribModel([], cAttrib)
                cAttrib.add(Attribute('removals', None, None, True, None, None, False, cremAttrib))
                for k in 'xysd' :
                    if k not in crem : continue
                    v = crem[k]
                    tmodel = AttribModel(v, cremAttrib)
                    cremAttrib.add(Attribute(k, None, None, True, None, None, False, tmodel))
            r = self.getColResults()
            if len(r) :
                crAttribModel = AttribModel([], cAttrib)
                cAttrib.add(Attribute('results', None, None, True, None, None, False, crAttribModel))
                for k in 'xysd' :
        ...
        if sres :
            ...
            resAttrib.add(Attribute('sequence', None, None, True, None, None, False, sAttrib))

        ures = []
        for i in range(len(self.user)) :
            ures.append(Attribute(str(i+1), self.getUser, None, False, None, None, False, i))
            
        uAttrib = AttribModel(ures, resAttrib)
        resAttrib.add(Attribute('user attributes', None, None, True, None, None, False, uAttrib))
        return resAttrib
